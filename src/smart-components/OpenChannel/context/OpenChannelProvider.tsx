...
interface OpenChannelInterface extends OpenChannelProps, MessageStoreState {
  // derived/utils
...
function OpenChannelProvider(props: OpenChannelProps) {
  const {
    channelUrl,
    children,
    useMessageGrouping,
  ...
  const fetchMore: boolean = utils.shouldFetchMore(allMessages?.length, experimentalMessageLimit);
...
const useOpenChannel = () => React.useContext(MessageContext);
